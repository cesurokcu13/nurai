// Turkish Color Names & Palette Mapping for Anonymous User Identity
const COLOR_PALETTES = [
  ...
  { name: 'Akuamarin', hex: '#14B8A6', bg: 'bg-teal-500/10', text: 'text-teal-400', border: 'border-teal-500/30' },
  { name: 'Menekşe Mor', hex: '#8B5CF6', bg: 'bg-violet-500/10', text: 'text-violet-400', border: 'border-violet-500/30' },
];

const ADJECTIVES = ['Nur', 'Halis', 'Müstakim', 'Münir', 'Safi', 'Latif', 'Aziz', 'Parlak', 'DUR', 'Ruşen'];

/**
 * Generates a random Turkish color nickname & hex badge color.
 * E.g., "Zümrüt Yeşil #482"
 */
export function generateRandomColorNickname() {
  const palette = COLOR_PALETTES[Math.floor(Math.random() * COLOR_PALETTES.length)];
  const randomNumber = Math.floor(100 + Math.random() * 900);
  const nickname = `${palette.name} #${randomNumber}`;
  return {
    nickname,
    hex: palette.hex,
    palette
...
/**
 * Returns consistent Tailwind style classes for a given nickname or color string
 */
export function getBadgeStyleForNickname(nickname = '') {
  if (!nickname) return COLOR_PALETTES[0];
  
  // Hash nickname to consistently select a palette index
  let hash = 0;
